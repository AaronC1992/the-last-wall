...
export interface SaveData {
  ...
  settings: { damageNumbers: boolean; screenShake: boolean; masterVolume: number; sfxVolume: number; musicVolume: number; graphicsQuality: GraphicsQuality };
...
export class SaveSystem {
  load(): SaveData {
    ...
    try {
      ...
      return {
        ...
        settings: { damageNumbers: parsed.settings?.damageNumbers ?? true, screenShake: parsed.settings?.screenShake ?? true, masterVolume: parsed.settings?.masterVolume ?? 0.5, sfxVolume: parsed.settings?.sfxVolume ?? 0.6, musicVolume: parsed.settings?.musicVolume ?? 0.3, graphicsQuality: parsed.settings?.graphicsQuality === 'low' || parsed.settings?.graphicsQuality === 'high' ? parsed.settings.graphicsQuality : 'medium' },
  ...
  private createDefault(): SaveData {
    return {
      ...
      settings: { damageNumbers: true, screenShake: true, masterVolume: 0.5, sfxVolume: 0.6, musicVolume: 0.3, graphicsQuality: 'medium' },
      completedCampaign: [],
    };
  }
}
