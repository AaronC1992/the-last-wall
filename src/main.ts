...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <main class="game-shell">
    <section class="battlefield">
      ...
      <div id="build-banner" class="build-banner" hidden><strong>BUILD PHASE</strong><span id="build-banner-action">Press SPACE to start battle</span></div>
      ...
      <button id="move-mode-button" type="button" class="move-mode-button" hidden>Move</button>
...
document.querySelector<HTMLElement>('#controls-hint-content')!.innerHTML = isMobileDevice
  ? '<span><i class="mouse left"></i>Tap to Select or Place</span><span><i class="mouse key">M</i>Tap Move, then Tap Map to Reposition</span><span><i class="mouse left"></i>Drag a Selected Tower to Aim</span><span><i class="mouse key">X</i>Delete mode removes towers on tap</span><span><i class="mouse key">Q</i>Tap Ability, then Tap Target</span><span><i class="mouse wheel"></i>Pinch to Zoom</span>'
...
document.querySelector<HTMLElement>('#skill-hints-content')!.innerHTML = isMobileDevice
  ? '<span><i class="mouse left"></i>Tap a Node to Buy</span><span><i class="mouse left"></i>Drag to Pan</span><span><i class="mouse wheel"></i>Pinch to Zoom</span>'
  : '<span><i class="mouse left"></i>Buy Upgrades</span><span><i class="mouse middle"></i>Pan Camera</span><span><i class="mouse wheel"></i>Zoom</span>';
document.querySelector<HTMLElement>('#build-banner-action')!.textContent = isMobileDevice ? 'Tap Start Battle when ready' : 'Press SPACE to start battle';
...
document.querySelector<HTMLButtonElement>('#move-mode-button')!.addEventListener('click', () => {
  game.toggleMoveMode();
});
...
new GameLoop((deltaTime) => {
  ...
  const moveModeButton = document.querySelector<HTMLButtonElement>('#move-mode-button')!;
  const removeModeButton = document.querySelector<HTMLButtonElement>('#remove-mode-button')!;
  const showMoveButton = isMobileDevice && game.currentPhase === 'build';
  const showRemoveButton = game.currentPhase === 'build';
  moveModeButton.hidden = !showMoveButton;
  moveModeButton.textContent = game.moveModeEnabled() ? 'Cancel Move' : 'Move';
  moveModeButton.classList.toggle('active', game.moveModeEnabled());
