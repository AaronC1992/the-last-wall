...
export class FlowField {
  readonly costs: Int32Array;
  readonly directions: Int8Array;
  readonly clearance: Uint8Array;
  private readonly queue: Int32Array;

  constructor(readonly grid: TerrainGrid, goal: MapPoint) {
    const size = grid.width * grid.height;
    this.costs = new Int32Array(size);
  ...
  rebuild(goal: MapPoint): void {
    ...
    while (head < tail) {
      ...
      const nextCost = this.costs[current] + 1;
      for (let direction = 0; direction < 4; direction++) {
        const neighbor = this.neighborAt(x, y, direction);
        if (!neighbor) continue;
        const neighborIndex = this.grid.index(neighbor.x, neighbor.y);
        if (this.costs[neighborIndex] >= 0 || !isWalkable(this.grid.get(neighbor.x, neighbor.y))) continue;
