...
const campaignEncounter = (level: number): CampaignEncounter => {
  ...
  if (level === 5) return { groups: [group(EnemyType.Grunt, 0.35, 0), group(EnemyType.Grunt, 0.2, 8, 'OPENING PRESSURE'), group(EnemyType.Runner, 0.15, 16), group(EnemyType.Brute, 0.15, 34), group(EnemyType.Armored, 0.15, 52, 'ARMORED COLUMN')], hpMultiplier: 1.1, speedMultiplier: 1.04 };
  ...
  if (level >= 16 && level <= 19) {
    const centerLane = level === 17 ? 'center' : level === 16 ? 'westCenter' : 'eastCenter';
    return { groups: [group(EnemyType.Armored, 0.22, 0, 'ARMORED FLANK', 0.25, 'west'), group(EnemyType.Brute, 0.16, 8, 'HEAVY PUSH', 0.25, 'west'), group(EnemyType.Grunt, 0.2, 18, 'CENTER ASSAULT', 0.2, centerLane), group(EnemyType.Runner, 0.2, 32, 'RUNNER FLANK', 0.3, 'east'), group(EnemyType.Exploder, 0.17, 46, 'EXPLODERS INCOMING', 0.3, 'east'), ...(level >= 18 ? [bossGroup(2, 66, 'BOSS APPROACHING', 'west')] : [])], hpMultiplier: 1.5 + (level - 16) * 0.06, speedMultiplier: 1.16 + (level - 16) * 0.02 };
  }
...
const CAMPAIGN_MAPS_BASE: readonly MapDefinition[] = [
  ...
  createMapDefinition('campaign-05', 'THE GATEWAY', [route({ x: 40, y: 1 }, { x: 40, y: 12 }, { x: 20, y: 24 }, { x: 20, y: 36 }, { x: 40, y: 42 }, goal), route({ x: 40, y: 1 }, { x: 40, y: 12 }, { x: 60, y: 24 }, { x: 60, y: 36 }, { x: 40, y: 42 }, goal)], [{ x: 40, y: 1 }], goal, settings('normal', 50000), 205),
...
export function validateCampaignEncounters(maps: readonly MapDefinition[] = CAMPAIGN_MAPS): string[] {
  ...
  for (const map of maps) {
    ...
    for (const group of groups) {
      ...
      if (typeof group.spawnPreference === 'number' && (group.spawnPreference < 0 || group.spawnPreference >= map.spawnCells.length)) errors.push(`${map.id}: invalid numeric spawn preference`);
      if (typeof group.spawnPreference === 'string' && group.spawnPreference !== 'random' && group.spawnPreference !== 'all' && map.spawnLabels?.[group.spawnPreference] === undefined) errors.push(`${map.id}: invalid spawn preference "${group.spawnPreference}"`);
