...
export class MapBuilder {
  ...
  private createDraft(): MapDefinition { return { version: 1, id: `custom-${Date.now()}`, name: 'MY VALLEY', width: DEFAULT_MAP_WIDTH, height: DEFAULT_MAP_HEIGHT, cellSize: DEFAULT_MAP_CELL_SIZE, terrain: Array.from(this.terrain), spawnCells: [], goalCell: this.goal, seed: Date.now(), enemySettings: { difficulty: 'normal', enemyCount: 500, variety: 'mixed' }, custom: true }; }
  ...
  private currentMap(): MapDefinition {
    ...
    return { ...this.draft, name, terrain, spawnCells: this.spawns, goalCell: this.goal };
  }
